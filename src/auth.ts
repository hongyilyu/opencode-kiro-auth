...
export async function readToken(): Promise<KiroToken> {
  const raw = await readFile(TOKEN_FILE, "utf8").catch(() => {
    throw new KiroAuthError(
      `kiro-cli token not found at ${TOKEN_FILE}. Run \`kiro-cli login\` (or \`kiro login\`) to sign in.`,
    )
  })
  const token = JSON.parse(raw) as KiroToken
  if (!token.accessToken || !token.refreshToken) {
    throw new KiroAuthError("kiro-cli token cache is missing accessToken/refreshToken. Run `kiro-cli login` to sign in again.")
  }
  return token
}
